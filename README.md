...
- **[skanfirmy.pl](https://skanfirmy.pl)** — free tools to verify Polish companies by tax ID (NIP), straight from official government registers. **9 tools**: VAT White List, KRS registry, REGON registry, bank account verification, tax micro-account, PKD 2025 codes, late-payment interest, EU VIES, and [bulk NIP lookup](https://skanfirmy.pl/bulk) (up to 30 tax IDs at once, CSV/JSON export). Every tool is also callable by AI agents via [MCP server](https://skanfirmy.pl/mcp) (9 JSON-RPC tools) or plain REST: `/nip/{number}`, `/nips/{list}`, `/regon/{number}`, `/vies/{country}/{number}`. No key, no login.
...
- **[polish-company-utils](https://github.com/bartosz-kuc/polish-company-utils)** — zero-dependency npm library: validate Polish NIP/REGON/PESEL/IBAN and generate the tax micro-account. The same checksum logic that powers skanfirmy.pl, extracted for developers.
- **[skanfirmy-mcp](https://github.com/bartosz-kuc/skanfirmy-mcp)** — the skanfirmy.pl MCP server documented for AI agents (9 tools, no key), with its Model Context Protocol registry manifest.
...
- **[skanfirmy.pl](https://skanfirmy.pl)** — bezpłatne narzędzia do weryfikacji polskich firm po numerze NIP, prosto z oficjalnych rejestrów rządowych. **9 narzędzi**: Biała Lista VAT, KRS, REGON, weryfikacja rachunku, mikrorachunek podatkowy, kody PKD 2025, kalkulator odsetek, VIES, oraz [zbiorcze sprawdzanie NIP-ów](https://skanfirmy.pl/bulk) (do 30 na jedno wywołanie, eksport CSV/JSON). Wszystkie narzędzia są też dostępne dla agentów AI przez [serwer MCP](https://skanfirmy.pl/mcp) (9 narzędzi JSON-RPC) i zwykłe REST-y: `/nip/{numer}`, `/nips/{lista}`, `/regon/{numer}`, `/vies/{kraj}/{numer}`. Bez klucza, bez logowania.
...
- **[polish-company-utils](https://github.com/bartosz-kuc/polish-company-utils)** — biblioteka npm bez zależności: walidacja NIP/REGON/PESEL/IBAN i generator mikrorachunku. Ten sam kod, który napędza skanfirmy.pl, wyciągnięty dla programistów.
- **[skanfirmy-mcp](https://github.com/bartosz-kuc/skanfirmy-mcp)** — serwer MCP skanfirmy.pl udokumentowany dla agentów AI (9 narzędzi, bez klucza), z manifestem do rejestru Model Context Protocol.
